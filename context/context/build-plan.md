...
- OAuth callback handler (`app/pages/auth/callback.vue`)
- Session management
- Global route middleware (`app/middleware/auth.global.ts`) protecting /dashboard, /profile, /find-jobs, /find-jobs/[id]
...
- Use the `@posthog/nuxt` module (added to `modules` in `nuxt.config.ts`) instead of a hand-rolled plugin. It mounts the browser client on app load and registers a Nitro server plugin (posthog-node, auto `shutdown()` on close) — covering both client and server in one integration.
- Configure under the module's `posthogConfig` key in `nuxt.config.ts`: `publicKey` from `NUXT_PUBLIC_POSTHOG_KEY`, `host` from `NUXT_PUBLIC_POSTHOG_HOST`. (Server-side flush tuning, when server events are added, goes under `posthogConfig.serverConfig`.)
- `app/composables/useAnalytics.ts` — typed wrapper over the module's `usePostHog()` exposing `track(event, props)`, `identify(user)`, `reset()`; SSR-safe (no-ops on the server). Its `AnalyticsEvent` union is the source of truth for event names and must stay in sync with the PostHog Events tables in `code-standards.md`.
- `posthog.identify()` called from `useAuth.refresh()` once a session resolves (userId + email, name, providers).
- `posthog.reset()` called on logout, after the `signed_out` event.
- Foundational events wired in this feature: `sign_in_started`, `sign_in_completed`, `sign_in_failed` (login + callback), `signed_out` (sign-out), `cta_clicked` (Hero, BottomCta, Navbar).

**PostHog events:** `sign_in_started`, `sign_in_completed`, `sign_in_failed`, `signed_out`, `cta_clicked`

> Server-side event capture for the agent funnel events (`job_found`, etc.) is added with features 10/13 — a `server/utils/posthog.ts` posthog-node client (the package ships with `@posthog/nuxt`) reading the same `runtimeConfig.public.posthog` values.
...
- Nitro route `server/api/profile.post.ts` saves all form fields to profiles table, called from `app/composables/useProfile.ts` via `$fetch`
...
- Form pre-fills with existing data on return visits via `useFetch`
- Profile data refreshed (`refreshNuxtData` or `useFetch` refresh) after save
