...
Jobizzy is a full stack AI-powered job hunting assistant. The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile using Gemini 2.5. For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier — company overview, tech stack, culture, why the role exists, and interview prep. The user reviews everything and applies with one click.
...
Jobizzy eliminates all of that preparation work. The agent finds the jobs, scores them intelligently against the user's actual skills, and researches each company so the user arrives at every application fully informed. The user just decides which ones to apply to and clicks.
...
- Two options on upload:
  - "Extract from Resume" → Gemini 2.5 parses resume and auto-fills profile form fields
  - "Skip" → resume stored as-is, profile unchanged
- User can manually edit any profile field at any time
- User can generate a clean professional PDF resume from their current profile data using Gemini 2.5
...
- Gemini 2.5 scores each job 0-100 against user profile
...
- Match score section:
  ...
  - Match reason paragraph from Gemini 2.5
- Company Research section:
  ...
  - Powered by Browserless + Stagehand browsing the company's public pages
...
- Gemini 2.5 synthesizes all extracted content into structured dossier
- Dossier displayed on job details page
- If company site cannot be found — Gemini 2.5 generates best dossier from company name and job description alone
...
- Resume PDF upload with optional profile auto-fill via Gemini 2.5
- Resume PDF generation from profile data using Gemini 2.5
- Adzuna API job discovery — searches by title and location, category filtered to IT jobs
- Gemini 2.5 job matching with score, reason, matched skills, missing skills
- Job details page with full structured description
- Company Research Agent — single Browserless browser browses company public pages, Gemini 2.5 builds dossier
...
- Gemini 2.5 match scores feel accurate and the reasoning makes sense
