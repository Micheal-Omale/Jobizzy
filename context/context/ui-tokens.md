...
Design tokens for Jobizzy. All colors, typography, spacing, and component values extracted from the delivered design. Use these exact values throughout the codebase — never hardcode colors or use raw Tailwind color classes in components.

---

## How to Use

This project uses **Tailwind CSS v4** via `@tailwindcss/vite`. All design tokens are defined using the `@theme inline` directive in `app/assets/css/main.css`. No `tailwind.config.ts` needed for colors or tokens.
...
## app/assets/css/main.css — Complete Token Definition
...
/* Maps every CSS variable above into a Tailwind utility-generating token */
@theme inline {
  ...
  --color-text-darker: var(--color-text-darker);
  --color-text-darkest: var(--color-text-darkest);
  --color-text-black: var(--color-text-black);
  --color-text-slate: var(--color-text-slate);
  --color-text-slate-medium: var(--color-text-slate-medium);
  ...
  --color-success-alt: var(--color-success-alt);
  ...
  --color-success-lightest: var(--color-success-lightest);
  --color-success-foreground: var(--color-success-foreground);
  --color-info: var(--color-info);
  --color-info-dark: var(--color-info-dark);
  --color-info-medium: var(--color-info-medium);
  --color-info-light: var(--color-info-light);
  --color-info-lightest: var(--color-info-lightest);
  ...
  --color-linkedin-light: var(--color-linkedin-light);
  --color-linkedin-foreground: var(--color-linkedin-foreground);
...
No dark theme is defined in this spec — the `@media (prefers-color-scheme: dark)` block in main.css stays empty until one is designed.
...
Font family: **Inter** — loaded via the `@nuxt/fonts` module.
...
- Font is Inter — always load via the `@nuxt/fonts` module, never use a fallback system font
