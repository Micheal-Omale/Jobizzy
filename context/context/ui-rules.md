...
Concise rules for building Jobizzy UI. Design assets are available — use them as the source of truth for visual decisions. These rules cover the most important patterns and constraints to keep the UI consistent without over-specifying every detail.

---

## Font

Always load Inter via the `@nuxt/fonts` module — never a manual `<link>` tag or system font fallback.
...
The `--font-sans` variable is already declared in `app/assets/css/main.css`. Apply `font-family: var(--font-sans)` on `body` (already wired in main.css). Never use system fonts as the primary font.
...
This project uses Tailwind v4 via `@tailwindcss/vite`. Tokens are defined with `@theme inline` in `app/assets/css/main.css` — no `tailwind.config.ts` needed. Never define colors in a config file. Always use `@theme inline` for new tokens.
...
- Never define colors in `tailwind.config.ts` — use `@theme inline` in `app/assets/css/main.css`
