...
function extractRootDomain(url: string, fallbackCompany: string): string {
  try {
...
export default defineEventHandler(async (event) => {
  ...
  if (job.source_url) {
    try {
      ...
      finalUrl = extractRootDomain(response.url, job.company || "company");
    } catch (e) {
  ...
  try {
    ...
    if (homepageData && (homepageData.oneLiner || homepageData.productSummary)) {
      ...
      const sortedLinks = (homepageData.pageLinks || []).sort((a: { kind: string }, b: { kind: string }) => {
    ...
    // Proceed to synthesis even if browser research failed entirely.
  } finally {
